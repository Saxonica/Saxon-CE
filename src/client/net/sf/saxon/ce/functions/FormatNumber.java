...
public class FormatNumber extends SystemFunction {
    ...
    public CharSequence evaluateAsString(XPathContext context) throws XPathException {
        ...
        //if (dfs == null) {
        DecimalSymbols dfs = null;
        DecimalFormatManager dfm = context.getController().getExecutable().getDecimalFormatManager();
        if (numArgs == 2) {
            ...
            try {
                ...
                dynamicError("Invalid decimal format name. " + e.getMessage(), "XTDE1280");
            }

            dfs = dfm.getNamedDecimalFormat(qName);
            if (dfs == null) {
                dynamicError("format-number function: decimal-format '" + lexicalName + "' is not defined", "XTDE1280");
            }
        }
            ...
            if (wholePartPositions != null) {
                ...
                for (int i = 0; i < n; i++) {
                    wholePartGroupingPositions[i] =
                            maxWholePartSize - (Integer) wholePartPositions.get(n - i - 1);
                }
                if (n > 1) {
            ...
            if (multiplier != 1) {
                try {
                    ...
                    value = (NumericValue) ArithmeticExpression.compute(
                            value, Token.MULT, new IntegerValue(multiplier), null);
                } catch (XPathException e) {
