...
public class Numberer_en extends AbstractNumberer {
    ...
    public String toWords(long number) {
        if (number >= 1000000000) {
            ...
            if (number < 13) {
                ...
                return englishUnits[(int) number] + "teen";
            }
            int rem = (int) (number % 10);
            return englishTens[(int) number / 10] + "ty" +
    ...
    public String toOrdinalWords(String ordinalParam, long number, int wordCase) {
        ...
        if (number >= 1000000000) {
            ...
            if (number < 13) {
                ...
                s = englishUnits[(int) number] + "teenth";
            } else {
                int rem = (int) (number % 10);
                if (rem == 0) {
                    s = englishTens[(int) number / 10] + "tieth";
                } else {
                    s = englishTens[(int) number / 10] + "ty" + tensUnitsSeparatorOrdinal + englishOrdinalUnits[rem];
                }
            }
        }
    ...
    private static String[] englishUnits = {
            ...
            "Ten", "Eleven", "Twelve", "Thir", "Four", "Fif", "Six",
            "Seven", "Eigh", "Nine"};
    ...
    public String monthName(int month, int minWidth, int maxWidth) {
        return abbreviate(englishMonths[month - 1], maxWidth);
    }

    private static String abbreviate(String name, int max) {
        return (name.length() > max ? name.substring(0, 3) : name);
    }
    ...
    public String dayName(int day, int minWidth, int maxWidth) {
        return abbreviate(englishDays[day - 1], maxWidth);
    }
